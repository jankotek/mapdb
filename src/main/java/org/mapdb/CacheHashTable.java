...
public class CacheHashTable implements Engine {
    ...
    protected AtomicReference<HashItem>[] items;
    ...
    public CacheHashTable(Engine engine, int cacheMaxSize) {
        this.engine = engine;
        this.items = new AtomicReference[cacheMaxSize];
        this.cacheMaxSize = cacheMaxSize;
        for(int i=0;i<cacheMaxSize;i++)
            items[i] = new AtomicReference<HashItem>();
    }
    ...
    public <A> long recordPut(A value, Serializer<A> serializer) {
        ...
        items[pos].set(new HashItem(recid, value));
        return recid;
    }
    ...
    public <A> A recordGet(long recid, Serializer<A> serializer) {
        ...
        if(value!=null)
            items[pos].compareAndSet(item, new HashItem(recid, value));
        return value;
    }
    ...
    public <A> void recordUpdate(long recid, A value, Serializer<A> serializer) {
        ...
        HashItem expected = items[pos].get();
        engine.recordUpdate(recid, value, serializer);
        items[pos].compareAndSet(expected, new HashItem(recid, value));
    }
    ...
    public void recordDelete(long recid) {
        ...
        if(expected!=null && recid == expected.key)
            items[pos].compareAndSet(expected,null);
    }
