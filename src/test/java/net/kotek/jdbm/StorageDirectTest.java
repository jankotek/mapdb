...
public class StorageDirectTest extends StorageTestCase {
    ...
    @Test public void long_stack_page_created_after_put() throws IOException {
        ...
        assertEquals(0, recman.phys.getLong(pageId)& StorageDirect.PHYS_OFFSET_MASK);
    ...
    @Test public void long_stack_page_overflow() throws IOException {
        ...
        pageId = pageId & StorageDirect.PHYS_OFFSET_MASK;
        assertEquals(8L, pageId);
        assertEquals(StorageDirect.LONG_STACK_NUM_OF_RECORDS_PER_PAGE, recman.phys.getUnsignedByte(pageId));
        for(int i=0;i< StorageDirect.LONG_STACK_NUM_OF_RECORDS_PER_PAGE;i++){
        ...
        assertEquals(1, recman.phys.getUnsignedByte(pageId));
        assertEquals(8L, recman.phys.getLong(pageId)& StorageDirect.PHYS_OFFSET_MASK);
    ...
    @Test public void test_freePhysRecSize2FreeSlot_asserts(){
        if(!CC.ASSERT) return;
        try{
            recman.freePhysRecSize2FreeSlot(StorageDirect.MAX_RECORD_SIZE + 1);
            fail();
    ...
    @Test public void test_freePhysRecSize2FreeSlot_max_size_has_unique_slot(){
        int slotMax = recman.freePhysRecSize2FreeSlot(StorageDirect.MAX_RECORD_SIZE);
        int slotMaxMinus1 = recman.freePhysRecSize2FreeSlot(StorageDirect.MAX_RECORD_SIZE - 1);
    ...
    @Test  public void test_freePhys_PutAndTake() throws IOException {
        ...
        assertEquals(arrayList(), getLongStack(StorageDirect.RECID_FREE_PHYS_RECORDS_START + recman.freePhysRecSize2FreeSlot(size)));
    }
